...
@onready var w_type = weapon.weapon_attributes.weapon_type
@onready var w_types = weapon.weapon_attributes.WeaponTypes

@onready var a_type = weapon.weapon_attributes.aim_type
@onready var a_types = weapon.weapon_attributes.AimTypes
		...
		a_types.PHYSICAL:
			match w_type:
				w_types.STEEL:
					...
					var bullet_dir = (gun_raycast.to_global(gun_raycast.target) - gun_raycast.global_position)
...
func splash_rum() -> void:
	while is_continuous == true:
		...
		var rum_impulse = Vector3(randf_range(-out_force, out_force), up_force, randf_range(-out_force, out_force))
		var rum_opening_up = rum_opening.global_transform.basis
		var final_impulse = rum_opening_up * rum_impulse
